...
void buzzer_control(int estado_equilibrio, int desvio, int estado_atual, int pino_origem) {
    ...
    if (pino_origem == MICROFONE_PIN) {
        ...
        if (estado_atual <= estado_equilibrio + desvio) {
            // Desliga os buzzers se estiver abaixo ou igual ao limiar
            pwm_set_gpio_level(BUZZER_PIN_1, 0);
            pwm_set_gpio_level(BUZZER_PIN_2, 0);
            return;
        }

        // Calcula o quanto o valor está acima do equilíbrio
        int excesso = estado_atual - (estado_equilibrio + desvio);
        
        // Mapeia o excesso para o intervalo PWM
        valor_convertido = map_range(excesso,
                                   0,                    // Mínimo excesso
                                   MAX_THRESHOLD - estado_equilibrio, // Máximo excesso possível
                                   0,
                                   MAX_PWM_LEVEL);
    } 
